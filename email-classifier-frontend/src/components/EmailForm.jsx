...
function EmailForm({ onAnalyze, isLoading }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-black bg-opacity-80 p-6 rounded-xl shadow-lg border border-gray-700">
      <div className="mb-4">
        ...
        <textarea
          id="emailText"
          rows="8"
          className="w-full p-3 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition text-gray-200"
          placeholder="Ex: Prezados, gostaria de saber o status do meu caso..."
          value={emailText}
          onChange={(e) => {
      ...
      <div className="mb-6">
        ...
        <input
          ...
          type="file"
          accept=".txt,.pdf"
          className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md 
        ...
        {selectedFile && (
          <p className="text-xs text-gray-400 mt-2">Arquivo selecionado: {selectedFile.name}</p>
        )}
      </div>

      <button
        type="submit"
        className="w-full bg-orange-500 text-black font-bold py-3 px-4 rounded-md 
                   hover:bg-orange-600 disabled:bg-orange-500 
                   disabled:cursor-not-allowed flex items-center justify-center transition"
        disabled={isLoading}
      >
        {isLoading ? (
          <>
